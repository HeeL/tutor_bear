...
describe Search do
  ...
  describe "#get_count" do
    it "returns zero when nothing found" do
      Search.new({who: 'teacher'}).get_count.should == 0
    end

    it "returns number of users" do
      5.times{FactoryGirl.create(:user, teach: true)}
      Search.new({who: 'teacher', no_price_set: true}).get_count.should == 5
    end

    it "doesn't limit count result" do
      (Search::PER_PAGE + 2).times{FactoryGirl.create(:user, teach: true)}
      Search.new({who: 'teacher', no_price_set: true}).get_count.should == Search::PER_PAGE + 2
    end
  end
  ...
  describe "#get_result" do
    it "returns result per page" do
      (Search::PER_PAGE + 2).times{FactoryGirl.create(:user, teach: true)}
      Search.new({who: 'teacher', no_price_set: true}).get_result.count.should == Search::PER_PAGE
    end

    it "considers the offset" do
      FactoryGirl.create(:user, teach: true)
      second_user = FactoryGirl.create(:user, teach: true)
      users = Search.new({who: 'teacher', offset: 1, no_price_set: true}).get_result
    ...
    context "who condition" do
      ...
      it "searches for teachers" do
        users = Search.new({who: 'teacher', no_price_set: true}).get_result
        users.count.should == 1
        users.first.teach.should be_true            
      end
      
      it "searches for learners" do
        users = Search.new({who: 'learner', no_price_set: true}).get_result
        users.count.should == 1
    ...
    context "price condition" do
      ...
      it "includes people without price" do
        users = Search.new({who: 'learner', no_price_set: true, price: 2}).get_result
        users.count.should == 2
      end

      it "excludes people without price" do
        users = Search.new({who: 'learner', price: 2}).get_result
        users.count.should == 1
      ...
      it "checks that the price is in the user price range" do
          ...
          Search.new({who: 'learner', price: 3}).get_result.count.should == 2
      end
    end

    it "doesn't exclude user if it was specified" do
    ...
    context "lang condition" do
      ...
      it "searches for people who has at least one specified langs" do
        users = Search.new({who: 'teacher', no_price_set: true, langs: 'Ruby,Python'}).get_result
        users.count.should == 2
      end

      it "returns all if langs empty" do
        users = Search.new({who: 'teacher', no_price_set: true, langs: ''}).get_result
        users.count.should == 2
      end

      it "searches by one lang" do
        users = Search.new({who: 'teacher', no_price_set: true, langs: 'C++'}).get_result
        users.count.should == 1
